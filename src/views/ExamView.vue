<template>
  ...
  <el-table height="526px" :data="examList">
    ...
    <el-table-column prop="creatorName" width="140px" label="创建用户" />
    <el-table-column prop="createTime" width="180px" label="创建时间" />
    <el-table-column label="操作" width="180px">
      <template #default="{ row }">
        <el-button
          v-if="isNotStartExam(row) && row.status == 0"
          type="text"
          @click="onEdit(row.id)"
          >编辑
        </el-button>
        ...
        <el-button
          ...
          type="text"
          @click="cancelPublishExam(row.id)"
          >撤销发布</el-button
        >
        <el-button
...
function onAddExam() {
  //跳转到新的页面上
  router.push("/foj/layout/updateExam?type=add");
}

async function onEdit(examId) {
  // await getExamDetailService(examId)
  router.push(`/foj/layout/updateExam?type=edit&examId=${examId}`);
  ///foj/layout/updateExam?type=edit&examId=${examId}
  ///foj/layout/updateExam?type=edit&examId=123
}
