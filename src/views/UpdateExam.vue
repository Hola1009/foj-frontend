          ...
          <el-table-column label="操作" width="80px">
            <template #default="{ row }">
              <el-button
                circle
                type="text"
                @click="deleteExamQuestion(formExam.id, row.id)"
              >
                删除
              </el-button>
            </template>
      ...
      <div>
        <el-dialog v-model="dialogVisible">
          <div class="exam-list-box">
            ...
            <!-- 题目列表 -->
            <el-table :data="questionList" @selection-change="handleRowSelect">
...
function addQuestion() {
  if (formExam.id === null || formExam.id === "") {
    // eslint-disable-next-line no-undef
...
async function publishExam() {
  await publishExamService(formExam.id);
  router.push("/foj/layout/exam");
}
...
function handleRowSelect(selection) {
  ...
  selection.forEach((element) => {
    questionIdSet.value.push(element.id);
  });
}
